...
from chat_main.models import ChatMessage
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if message.strip() == "@AI what was my last timed out reason":
            ...
            if not record:
                await self.send(text_data=json.dumps({
                    "message": "You have not been timed out recently."
                }))
                return
        ...
        if is_timed_out:
            await self.send(text_data=json.dumps({
                "message": f"You are timed out for {remaining_time} more seconds."
            }))
            return

        channel_info = await get_channel_info(self.room_name)
        ...
        if llm_response["status"] == "approved":
            ...
            await self.send(text_data=json.dumps({
                ...
                "relevance": "Relevant as per channel description"
            }))

        elif llm_response["status"] == "timeout":
            await self.log_timeout(self.user_id, self.room_name, llm_response.get("reason", ""), message)
            await self.send(text_data=json.dumps({
                "message": "You have been timed out for violating channel rules."
            }))

        elif llm_response["status"] == "banned":
            await self.log_ban(self.user_id, self.room_name, llm_response.get("reason", ""), message)
            await self.send(text_data=json.dumps({
                "message": "You have been banned for violating channel rules."
            }))
            await self.close()

        else:
            await self.send(text_data=json.dumps({
                "message": "Your message was removed for being off-topic. Please stay on the channel's subject."
            }))
            return
    ...
    def log_timeout(self, user_id, channel_name, reason, message):
        UserModeration.objects.create(
            ...
            timed_out_reason_message=message,
            timed_out_initial_time=timezone.now(),
    ...
    def log_ban(self, user_id, channel_name, reason, message):
        UserModeration.objects.create(
            ...
            banned_reason_message=message,
            banned_at=timezone.now()
        )

    @database_sync_to_async
    def is_user_timed_out(self, user_id, channel_name, now):
        entry = UserModeration.objects.filter(
            user_id=user_id,
            channel_name=channel_name
        ).order_by('-timed_out_initial_time').first()
    ...
    def get_last_timeout(self, user_id, channel_name):
        return UserModeration.objects.filter(
            ...
            channel_name=channel_name,
            is_banned=False,
            timed_out_reason__isnull=False
        ).order_by('-timed_out_initial_time').first()
